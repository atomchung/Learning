...
updated: 2026-07-26
last-session: 2026-07-26 使用者傳 SemiAnalysis 付費文章截圖(《Can AMD break the CUDA Moat?》)要求展開研究→追問 DeepSeek/華為洩漏演講的證據強度→追問開發者生態量化指標(多少人投入華為/AMD 開發)。三輪深挖產出 `notes/cuda-moat-2026.md`(四層證據:AMD 官方敘事/DeepSeek-華為洩漏敘事真偽鏈/學術 benchmark/開發者生態指標)。**核心判斷**:CUDA moat 變淺但沒填平,**中系(華為/DeepSeek)比美系(AMD)落後得更多、證據也薄得更多**——AMD 有 MLPerf 中立跑分+HIP 轉譯層實測僅 2% overhead 撐腰;梁文鋒洩漏投資人交流會連文件真偽都未經 DeepSeek/Bloomberg 證實,是目前查過「作者自報 vs 獨立複現」證據最弱的一次(比一般官方宣稱還弱一層),但內部一致性檢查(10個月回本/6倍毛利 對得上自家 2025-03 公開 545% 毛利率)給了一個可信度錨點。學術量化(CUDABench)潑冷水:LLM 生成 CUDA 編譯成功率高但功能正確率低,「AI 輕鬆寫生態」的樂觀論還沒被證實。開發者生態量化印證落差:CUDA 職缺 165,000+ vs ROCm 82(LinkedIn,量級差 2000:1)、華為「生態會員」6.65M vs 窄義 CANN Next 僅 15,000+(2026底自估)——廣義生態大、真正做底層 kernel 開發的窄很多。已加入預測帳「CUDA moat 到底多快瓦解」(`check:2027-01`)。踩的坑:直接搜「XX vs YY comparison」容易只撈到二手轉述,要加「academic/arXiv/independent」關鍵字才挖到 CANN Bench/CUDABench 這類真正的量化研究。**（2026-07-20 舊 session 詳情已下沉,見 inbox.md 同日條目;durable 判斷已收進下方話題/預測帳,profile 只留 last-session 給最近一次）**
...
- **AI 產業判斷 / 投資訊號** — 讀信號不讀數字、開源商品化計時器、價值流向、中國開源、模型軌跡。核心：記憶體＝三個市場非一個（commodity DRAM/HBM/NAND 定價邏輯不同，混講必誤判）；元判斷「商品週期盯 capex 紀律不盯需求故事」。最新(2026-07-20)：Mercor take-rate 出現**反向**訊號(labs 未內部化、外包擴大，客戶集中度＝天花板，見預測帳)；記憶體三市場活教材＝SK 海力士 7/2 撤長約價格上限與美光分道；hyperscaler capex 共識 $7,250 億(+77%)未轉向、辯論退到折舊會計、真檢驗點＝月底 Q2 財報；Kimi K3(2.8T 史上最大開源)計時器加速但官方自評待第三方；**技術拆解(同日兩場合流)**：混合注意力(KDA 3:1)+AttnRes 兩招才是「參數大不等比貴」的機制(MoE 管**參數軸**、KDA 管**長度軸**，兩軸不可混講)，線性注意力位置編碼弱點被 NoPE 反超解決＝非妥協；**另一場補的成本/證據面**：真決策是**稀疏度 98.2%**(896 專家取 16、活躍 ~50B，每 token 算力≈DeepSeek V4 Pro 的 49B)，貴 17 倍是卡在「**擺得下的門檻**」(~1.4TB HBM、需 64+ 加速器)不是算力；AA 自購 API 實測**極度囉嗦**(130M vs 平均 63M output tokens)＝**實際單位任務成本約名目價兩倍**；官方跑分 harness 不對等(K3 用自家 KimiCode、對照用 Claude Code)，可信的是人類盲測 LMArena 前端 #1、但一般文字榜僅 1486（`notes/kimi-k3-hybrid-attention.md`、`notes/mercor-rlhf-labor-market.md`）。
...
- **eval 生態位**（`check:2026-09`）→**命題拆兩層**：promptfoo 被 OpenAI 收編坐實(2026-03,~$86M,被窄化成 red-team)＝單一中立工具易被收編；但中立層換骨不消失,由政府(UK AISI Inspect)/基建大廠(ClickHouse 收 Langfuse、續 MIT)/獨立 vendor 三腿撐厚。判準卡再升級：瓶頸從「寫 rubric」→「judge 在專業領域穩定套用+對齊人類」(arXiv《Learning to Judge》2026-02,專業域相關係數崩到<0.3)。**→ 結算訊號（07-05 換層）**：開門＝出現綁 policy+audit 的 per-action 計價 eval pure-play 拿到規模營收；關門＝GitHub/orchestrator 把 eval/reconcile 原生內建。**最新(2026-07-20)**：關門訊號**半觸發**——微軟 Copilot Studio 把 Agent Evaluations 原生內建 GA(7/7)，但只關 walled-garden 內(對你在該平台建的 agent)；同期 judge 可靠性論文(專業域對 SME 一致性掉 60-64%)推高「中立+專業域對齊」門檻＝**你押的窄縫反而更硬**（`notes/eval-ecosystem-niche.md`、`llm-call-niches` 卡）。
- **安全/紅隊 eval 值不值得當個人差異化深入？**（`check:2026-08`）→**傾向 ✅（兩面實錘，2026-07-20）**：頂尖攻擊能力鎖模型廠內部（OpenAI 發 GPT-Red、prompt injection 攻擊成功率 84% 但**明言不釋出**；Anthropic Mythos 亦僅 vetted 釋出）＝攻擊側封閉；個人差異化落「驗證/整合/合規」側已成形——EU AI Act 8/2 大限催出獨立紅隊顧問市場($125-200/hr、$130-200k/年，吸案 HiddenLayer/Mindgard/10a Labs)。**→ 結算訊號（07-05 有時限檢索）**：到期主動掃案例(求職/顧問/開源三處)，掃到＝✅——**本次已掃到顧問+求職實例**（`notes/anthropic-blog-2026-06.md`、inbox 2026-07-20）。
...
- **Mercor/RLHF 專家勞動供應鏈的 take-rate 會不會被 labs 內部化壓縮？**（2026-07-11，`check:2027-01`）追蹤中。GMV $20 億(2026-06)≠淨營收 $6-8 億；估值談判中衝 $200 億(2026-07-09)。**→ 結算訊號**：OpenAI/Anthropic 等是否加速自建全職 domain expert 團隊(內部化訊號)，或 Mercor 財報/融資揭露顯示 take-rate 被壓縮；反向則是外包持續擴大＝供應鏈層站穩。**最新(2026-07-20)：反向訊號**——labs **未**內部化(專搜查無自建 domain expert 團隊)，中介反往上爬(併 Deeptune 做 RL 訓練環境)、labs 用腳投中立(Surge 傳 $250-300 億當默認選項)＝外包擴大；**但補脆弱性**：Mercor 3 月遭 LiteLLM 供應鏈攻擊外洩 4TB、最大客戶 Meta 無限期暫停＝**客戶集中度就是 take-rate 天花板**；治理黃旗：Deeptune 天使投資人自承支票「一開始就為併購」＝關係人交易訊號（`notes/mercor-rlhf-labor-market.md`、inbox 2026-07-20）。
...
- Harness 影響大於換模型 → `topics/coding-agents/cards/harness-beats-model.md`（Salesforce +79% 來自 review agent+skills；Google DevRel 也淡化 model＝利益相反方驗證；**2026-07-20 加 failure-class 天花板**：「選錯工具」那類的 harness 側修補上限 **23%**，因為錯在 decision readout 不在資訊取得）
...
- 知識系統勝負手＝預編譯本地索引、按需 page-in，不重塞 context → `topics/coding-agents/cards/precompile-to-local-index-not-restuff-context.md`（2026-06-21 週綜合；**2026-07-20 邊界再修正**：「塞得下」≠「用得到」——累積量進 rot 區間就該動，且 context 壓縮＝**可逆降級不是遺忘**、pager 該是模型自己）
- Verifier 是階梯不是開關，紅利只在沒有執行回饋的任務出現 → `topics/coding-agents/cards/verifier-is-a-ladder-not-a-switch.md`（2026-07-20 論文掃描；把「loop 瓶頸是 verifier」從二元升級成「階梯高度 × 任務相依」兩問。可直接用的判準：環境自己會報錯→投資 verifier 邊際報酬低；無執行回饋的知識判斷型→verifier 才是主槓桿）
