...
updated: 2026-07-17
last-session: 2026-07-17 Threads 截圖討論「Fable 流程贏不是模型贏」(fable-method skill 把 Claude Fable 5 工作流拆成弱模型可跑的 skill)——拆解成「地板 vs 天花板」判斷:蒸餾工作流墊高弱模型的地板(省流程錯誤),但封頂在弱模型自身的天花板(難判斷、品味),故工作流價值與模型強度成反比(可證偽檢定:若對強弱模型幫助一樣大＝抬天花板不是地板);那句 "the eval that keeps..." 是真正的轉移機關,但威力被 verifier 上限鎖死→退化成 one-sided-checks-drift。升新卡 `workflow-lifts-floor-model-sets-ceiling`(coding-agents 第 18 張),填 harness-beats-model(墊地板效益大)與 model-capability-capped-by-harness(harness 往下掐)之間缺口。順手記:po 說 1314 星、卡片顯示 2k 星,數字對不上＝「讀信號不讀表面數字」即時案例。
...
- 其餘卡見 `README.md` 地圖（~26 張：ai-industry-reading 9、coding-agents 18）
