...
### addyosmani 自己的檔案結構:三層正交(2026-07-08 查證,修正上節推斷)

WebFetch addyosmani/agent-skills 真實結構:頂層 `README.md` / `AGENTS.md` / `CLAUDE.md` / `plugin.json` + 目錄 `skills/`(24 SKILL.md)、`agents/`、`.claude/` `.gemini/` `.codex-plugin/` `.opencode/`(各工具適配)、`evals/` `hooks/` `commands/`。

**修正**:上節推斷「addyosmani 的 AGENTS.md 是給不認得 skill 格式的工具的通用橋」——**錯,那是 fomo-kernel 的用法**。addyosmani 的 `AGENTS.md` 是**角色層**:定義專家 persona(code-reviewer / test-engineer / security-auditor / web-performance-auditor)+ 它們怎麼協作的編排。

**addyosmani 是三個正交維度各一層**:
- **能力層(`skills/` SKILL.md)**:24 個 workflow —「**怎麼做**」(步驟 / 驗證關卡 / anti-rationalization 表)。
- **角色層(`agents/` + `AGENTS.md`)**:專家 persona + 編排 —「**誰做、怎麼協作**」。
- **適配層(`.claude/` `.gemini/` `.codex-plugin/`…)**:同一套內容封裝成各工具原生格式 —「**在哪跑**」。skills 是純 markdown,單一 source-of-truth 裝到 70+ 工具。

**skills vs agents 分兩層的思路 = 「skill vs agent」概念的目錄化**:agent = 角色/判斷主體,skill = 被調用的能力。分開因為**多對多**——一個 persona 調多個 skill、一個 skill 被多個 persona 用,綁死就無法複用。

**對比 fomo-kernel(呼應姊妹判準)**:addyosmani 多專家視角(review/test/security/perf 各一 persona)→ 需要角色層 `agents/`;fomo-kernel 只有**一個**角色(交易教練,單一鏡片)→ **角色層退化成 1,沒有 `agents/`**,它的 `AGENTS.md` 挪作路由橋。**同名 `AGENTS.md` 兩 repo 完全不同義。** →**姊妹判準**:前面立「skill 數 = 使用者會單獨想要的動詞數」,這裡補「**agent(角色)數 = 需要獨立協作的專家視角數**」,fomo-kernel = 1。

### 已拍板的架構決策(2026-07-08)

- **對外單一入口 = 定案**(版本 B:合一入口 + agent 內部路由;從「建議」升格成 owner 拍板)。
- **eval「判斷的交派紀律」= 獨立議題**,之後專門開 issue 討論(現在不深入)。真開 GitHub issue 屬 fomo-kernel repo 的對外動作(先 `gh issue list` 查重),留待 fomo-kernel session。
