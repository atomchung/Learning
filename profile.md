...
updated: 2026-07-08
last-session: 2026-07-08(續)回到上次擱置的「fomo-kernel 怎麼借鑑 addyosmani」,辨析「拆多 vs 收斂一 skill」由 domain 結構決定:三判準(階段=獨立能力 vs 一事多工序 / 誰編排=使用者 vs 產品 / 狀態=檔案 vs 記憶迴圈),skill 數=使用者會單獨想要的動詞數(coding 8、復盤 1)。結論 fomo-kernel 多已獨立收斂(其 research §28),addyosmani 用途=背書漸進載入+反向驗證單一入口+別丟自家 eval 強項;沉澱進 notes/addyosmani-agent-skills.md。開場預測「該學它拆 slash command」被 §28 打臉。續問「要靈活還是一起用」精化「誰編排」判準:意圖權給用戶(靈活表達)、編排權系統扛(打包);靈活該在意圖層不在工具層(顯式 vs 隱式靈活),investment_note 13-skill 平鋪 = 工具層靈活的反面教訓。用戶要求之後深入討論 fomo 架構(見候選池)。(07-07 Fable/Gemini/Codex 交叉審查仍待補記,見 inbox 同日)
...
- **fomo-kernel 架構深化（用戶 07-08 明確要「之後討論架構」）**：拆多 vs 收斂一已辨析完（`notes/addyosmani-agent-skills.md`「對照 fomo-kernel」節）。待續三題：①意圖路由做強（research §27 的 `on user_intent` 多 arm + `on schedule`/`on market_event` 非對話觸發）②pre-trade gate 當唯一值得的第二入口③SKILL.md ~27k 瘦成 dispatcher <2k + mode 子檔案觸發才載（§28 已定未做）。屬 fomo-kernel repo 的活，認領才動（跨 repo + 並行紀律）。
