...
**我猜的**：揭露規則不等於一致性檢查。**方向對但淺一層**——真正的問題不是檢查不夠強，是**檢查跑在人排序之後、且不進迴圈**。

### 發現一：排序與正確性正交，而排序是唯一回饋訊號

- 排第一那則：核心類比承重點方向反了，出處欄還老實附了正確的來源連結——**它去核對了外部來源，但沒去問「我自己的記錄有沒有記過同一件事」**
- 排最後那則：引用準確率四則最高（19 條可核 18 對），唯一主動聲明自己兩個數字口徑不同不可互換
- 三則答案引同一場財報，給出三個不同數字——**外部事實在這套流程裡沒有共同的地面**
- 稽核抓到那條錯誤是運氣：判準是「本地找得到對照物且相符＝對，找不到＝不算錯」，91 條裡 37 條落在「不算錯」
- **時序才是致命的**：稽核是隔天補跑的，排序當下那個錯誤還不存在於任何檔案裡

→ 沉成卡片 [correctness-gates-before-human-preference](topics/coding-agents/cards/correctness-gates-before-human-preference.md)。**判斷句用他的措辭**（我停在描述現象「系統會往更會說服演化」，他給的是解法「正確性交給機器把關、不依賴人判斷」）。這是第二次他的命名比我的準。

### 發現二：那個 skill 不是被禁止聰明，是入口就關了

我先說「skill 明文禁止它說為什麼買某檔」——**查證後推翻，講太滿**：那句禁令的主詞是另一條路線（週度復盤卡），而考慮交易那條路明文**要求** build case for and against。歸屬錯誤，與 2026-08-10 同型。

修正後的定位：**限制在輸入不在輸出**。沒有標的就啟動不了，三層市場查詢全掛在標的上（所以它查得比別人少），只能把既有候選逐個丟進去秤，四個候選回來四面牆，論述層只能綜合成「等」。

**使用者再修正一層（比我準）**：不是參數層，是**入口宣告**——skill 描述自己能做什麼的那段，所有例子都預設已經有標的。「我有一筆錢要投什麼」在讀到任何輸出規則之前就被判出界了。

### 附帶結論：限制不是一律有害，分界在有沒有機制

- **有機制的**（數字只能來自引擎、外部事實必帶來源、AI 判斷要標記）→ 換來的是實測最高的準確率
- **只有指令的**（不准給價格目標、不准選一檔）→ 模型面對模糊禁令往最保守端塌陷，換來沉默

→ 能機械化的就機械化，不能機械化的就別寫成規則靠自律。

### 落地（三個 issue 串成一條鏈）

- 新開 [#825](https://github.com/atomchung/fomo-kernel/issues/825)：資金部署問題無路可走；入口宣告是第一層限制，premise 契約第二層，判斷天花板第三層
- [#716](https://github.com/atomchung/fomo-kernel/issues/716) 補證據：它要的「不低於通用模型的基準」現在有實測了，而且結果證明它的診斷（缺中間層）成立
- [#610](https://github.com/atomchung/fomo-kernel/issues/610) 補證據：**它的四項驗收全過，產品仍是四個裡最差的**——存在性檢查量不出說服力也量不出準確性
- 關係：610 管絕對地板 → 716 管相對基準 → 825 管具體機制
...
## 2026-08-31 — Claude $100 vs $200 方案：週限制真的只差兩倍嗎

使用者傳 Facebook「Claude Taiwan」社團貼文截圖（張峻瑜發，25 讚 5 留言），主張「$100 說是 5x Pro、$200 說是 20x Pro，但這僅限於 5 小時限制；每週限制 $200 只是 $100 的兩倍而已」，問「這是真的？」

**我先寫的預測**：方向對、數字略糙。「5x/20x」是行銷標籤，貼在 5 小時 session 額度上；2025-08 加的週限制兩方案差距遠小於 4x，猜落在 **1.5–2x**。

**對答案：預測命中，且貼文的「兩倍」還高估了一點。**

**① 倍數掛在 5 小時窗口 — 貼文說對了。** Anthropic 官方措辭就是 "Max offers 5x or 20x the usage of Pro **per 5-hour session**"。倍數綁 session 窗口，不綁週限制。

**② 週限制比值 ≈ 1.7x，不是 2x 也不是 4x。** 流通的 Claude Code Sonnet 週時數估計：Max 5x 約 140–280 小時／週、Max 20x 約 240–480 小時／週。有使用者回報 Anthropic 客服確認週增幅約 1.7x。

留言區「季文淵」那條——「5 小時的 20X 有比較長，但 5X 跟 20X 周用量差不多」——是整串最準的判斷。session 內爆發力確實差 4 倍（約 225 vs 900 訊息／5h），週總量沒有等比放大。

**③ 貼文漏掉的時效層：這些數字正在變。**
- 2026-05-06 — 5 小時限制永久加倍，並移除 Pro/Max 的尖峰時段降額
- 2026-05-13 起 — 週限制臨時 +50%，延期四次
- 目前 +50% 撐到 **9/13**
- **9/14 起** — 基準永久 +25%，但相對「現在」等於 **−17%**

**④ 可重用的元判斷（升卡候選）**：**行銷倍數會掛在最好看的那層 metric 上。** 這裡有兩個閘門（session ＋ weekly），廠商挑 session 標倍數因為那層差 4 倍；真正決定月底夠不夠用的是 weekly，那層只差 1.7 倍。買方該問的不是「倍數多大」，而是「**這個倍數綁在哪個 binding constraint 上**」。接 `notes/info-intake-routine.md` 的第一個篩子（信號 vs 表面數字）——這裡的「20x」就是表面數字。

**⑤ 證據降級（重要，比結論本身更該記）**：
- `support.claude.com` / `claude.com` / `www.anthropic.com` **三網域全被本環境 egress 擋**，WebFetch 全滅（EGRESS_BLOCKED），連 `morphllm.com`、`bleepingcomputer.com` 這些二手站也擋。**全程只剩 WebSearch 摘要**。
- 更關鍵：**Anthropic 自 2026 起已停止公佈固定的小時數／訊息數**，只講相對容量。網路流傳的 140–280 / 240–480 是 **2025-08 首次上線時的舊估計**，現在已非官方數字。
- 所以「1.7x」這個比值本身也是二手推算，**不是官方數據**。唯一可信的一手來源是使用者自己帳號的 Settings → Usage。
- → 這是 env-403 第 3 次；且這次是 **credibility-miss 的邊界案例**：三個獨立二手來源互相引用同一組 2025 舊數字，看起來像交叉驗證，實際是同源迴音。已記 `meta/defects.md`。

**⑥ 這次沒踩但差點踩的坑**：`claude-api` skill 的 TRIGGER 寫「使用者問 LLM limits 就必讀、不准憑記憶答」，我照載了，但它整份講的是 API/SDK（token、pricing per MTok、rate limits），**跟消費端訂閱方案的 5 小時／週限制是兩套東西**。載入後立刻判定不適用並改走查證路線是對的——但這代表該 skill 的 description 把「limits」寫太寬，會誤觸。屬 harness 層，已記 defects。

**追問（同場）：「如果我的周用量效率最高，應該買 max 100 usd 的就好？」**

**答：是——但前提是「互動式使用 + 週量吃滿」，且溢出要走額外點數而非升級。**

**邊際帳**：多付的 $100 週限制只換到約 +0.7x，每單位變貴。而 Pro / Max 5x / Max 20x 都能開**額外用量點數**（extra usage credits，超出方案上限後按標準 API 費率計費，個人帳號每月最多買 $2000 折扣包）。→ **「按用量溢出」在「知道自己會超、但不知道超多少」時，永遠不劣於「預付固定容量」**。這是比 1.7x 本身更可重用的一條。

**兩個會翻掉結論的條件**：
1. **其實是被 5 小時窗口卡住**（同一天內反覆撞「resets in X hours」）＝ session-bound，$200 那 4x 真正住這裡
2. **有跑 headless／腳本化**——這是**第三個計量表**，我第一次答漏了：2026-06-15 起有獨立的**每月程式化額度池**（Pro $20／Max 5x **$100**／Max 20x **$200**），只有 Agent SDK、`claude -p`、GitHub Actions、第三方 API app 會扣，**不吃互動式週限制**。這層是乾淨的 2x 且以美元計價 → 有排程 agent/CI 的話，多給的 $100 程式化額度可能遠比週限制那 0.7x 值錢。

**排除的套利**：2 個 $100 帳號 = $200 換乾淨 2x 週限制，但違反一人多帳號條款、且無法共用 session/context（Claude Code 認證綁帳號）。非可行選項。

**我第一次答的缺口（值得記）**：我第一輪只比了「session vs weekly 兩層閘門」，**漏掉第三層**。原因是第一輪的框架來自貼文本身（貼文只講兩層），我沿用了它的維度而沒自己重數一遍計量表。**這是「沿用提問者的框架」型的漏查**——跟 08-10「先有敘事框架、後補證據」同源，但形態不同：那次是自己編框架，這次是借用對方的框架。是使用者追問「那我該買哪個」才逼出第三層。→ 記 defects。

**結論給使用者**：先看 Settings → Usage 確認撞的是哪道牆（唯一一手數據）。互動為主 + 週量吃滿 → $100 + 額外點數；有程式化工作流 → $200 才開始合理。時效提醒：9/14 起基準永久 +25%、相對現在 −17%，對兩方案大致等比例，不改比較邏輯但絕對餘裕會縮，要試 $100 建議 9/14 後再評估。

**再追問（同場）：確認「只看週用量的話 $200 就是 $100 兩倍」／「不是週用量四倍」**

**修正一：不要記成「剛好兩倍」。** 最佳估計 **~1.7x**，而**那 0.3 的差就是論點本身**——若真是 2.0x，付兩倍拿兩倍、$/單位持平＝完全公道，貼文就沒什麼好批評的；實際 1.7x ＝ $/單位貴約 15%。**四捨五入成「兩倍」會把批評的立足點一起抹掉。**

**修正二：「週用量」不是單一計量表。** Max 有**兩個**週上限（跨全模型 + Sonnet 專用）。用 2025-08 舊區間算：
- Sonnet 週：140–280 → 240–480 小時 ＝ **1.71x**
- Opus 週：15–35 → 24–40 小時 ＝ **1.14–1.6x**
→ **Opus 重度使用者的週差距比 1.7x 更窄**，最糟接近 1.1x（付兩倍錢多拿一成）。

**「4 倍」到底住在哪（完整對照，四個計量表只有一個是 4x）**：
- 5 小時 session 窗口 → **4x**（$200 vs $100，即 20x vs 5x Pro）← 唯一
- 週限制 Sonnet → ~1.7x
- 週限制 Opus → ~1.1–1.6x
- 程式化額度池 → 剛好 2x（$100 vs $200）

→ 多付的 $100 買到的是**單次能連續跑更久**，不是整週能跑更多。

**證據強度分層（這次特別要分清）**：
- **硬**：週限制差距遠低於 4x、且低於 2x。方向穩。
...
## 2026-09-16 — Decision Learning 的最小實作：`/learn` skill + 三個 dry run

**問**：接 09-15 的 `notes/decision-learning-from-wondering.md`。使用者要求在 Learning repo 上直接做（不另開 repo、不拆成 investment 專案），先做一個**可以在 Claude Code 直接用的輕量 learning workflow**，驗證：模糊問題丟進來 → 先找出最少需要理解的幾件事 → 一次教一小步 → 可追問/講深/跳過/branch 再回主線 → 複雜度依問題自適應 → 最後才收束成理解/未知/關鍵判斷，有決策需求才延伸。**明確禁止**做成 FOMO Kernel 那種重流程（開場逼填 thesis/evidence/falsifier/confidence、一串澄清問題、固定 schema、長分析），也不要 React / DB / multi-agent / vector DB。成功標準只有一條：**比普通 chat 更容易一步步搞懂，且不增加使用負擔。**

**做法**：只寫一個檔 `.claude/skills/learn/SKILL.md`（97 行 / 5.5KB，對照被批評的 fomo-kernel SKILL.md ~27KB），零基礎設施。關鍵是**把 09-15 那份的 schema 全部藏到後台**——decision-learning 的 must_understand / claim-evidence-implication / falsifier 都還在，但一個都不准出現在前台。

**三個設計決定（都是為了「不增加負擔」）**
1. **檔位判準只有一句話**：「要改變他現在的判斷，最少需要幾個他還沒有的東西？」——這個數字就是步數。0→直答、2-3→小徑、4-7→完整、>7→這是三個問題。**寧可往低猜一檔**（嫌淺的會說話，嫌重的直接不用）。這一句同時取代了原設計的 must_understand / nice_to_know / skip 三層 YAML。
2. **開場三行內就開始教，不問背景**。唯一能問的情況也不准停下來等，要把假設寫進句子裡繼續講（「先當你是自用不是要投資，錯了跟我說——」）。這條是整個 skill 的核心反重流程機制：澄清問題是負擔感的最大來源。
3. **收深度看檔位、加不加決策段看問題本身**——兩軸分開。這條是 dry run 當場撞出來的（見下）。

**Dry run（三個真實問題，使用者回應由我模擬，這點在 note 裡標明了）**
- **檔位 0**（skill vs slash command）：5 行答完就停。**誠實發現：這裡 skill 的價值是 0，它的工作就是不要上場。**
- **檔位 1 + 帶決策**（內部知識庫要不要從 RAG 換成 agent 直接搜檔）：3 步 + 一次挑戰（五千份 grep 會不會慢）+ 決策收尾。**撞出規則打架**：我原本寫「檔位 1 的決策段就兩句」，實際三句才對——**硬性字數是假精確**，改成「短」。
- **檔位 2 + 投資 + 岔題**（記憶體這波還能不能追）：5 步 + HBM/DRAM 岔題 + 反事實檢查 + 三段收束 + 決策段。底材用 repo 既有的 `memory-industry-map` / `cxmt-dram-challenge`。

**這次最重要的一條規則**（不是流程，是誠實）：**需要即時數字（價格、市佔、最新財報）就說「這個要查」，不要憑記憶講。** 檔位 2 跑到第 3 步時它當場擋下一次 **rot**——repo 的記憶體地圖 freshness 標 2026-06/2026-08，今天 9 月中，拿那份講現在的價格就是拿過期卡當現在的事實。這條規則是 CLAUDE.md 的缺陷分類直接長出來的，不是我想像的。

**反膨脹（使用者要求「能刪就刪」，我把閘套回自己身上）**
- 刪：「他說跳過→跳掉」「他自己講出結論→往下走」——**模型不寫也會做，寫了只是膨脹**
- 刪：「這個 skill 沒解決的」整節（對行為零影響）→ 壓成開頭一句
- 刪：「不要做的事」六條 → 三條（六條裡有五條是上面已經用粗體講過的重述）
- 加：超過 3 件就條列別擠一行（手機可讀性）、需要即時數字要標查證
- **淨結果 99 行→97 行、5.3KB→5.5KB，幾乎沒變小。這點誠實講**：加的兩條跟刪的體積相抵。真正收穫不是變短，是留下的每一條都通過了「不寫也會發生嗎」這一問——這一問本身也寫進 skill 末尾，給未來維護的人。
- **本來要刪、dry run 證明該留的**：反事實檢查。我原本覺得它最像考題準備砍掉，結果它是檔位 2 最有價值的一刻（接在最後一步後、用「想聽你怎麼看」收尾，沒有考題感，還當場驗出有沒有抓到主線）。

**預測帳**：開場我猜「最小實作應該是 skill 而不是 slash command，而且難的是 schema 設計」——**後半被自己打臉**。schema 幾乎照搬 09-15 那份就成立；真正難的、也是三次修改全部集中的地方是**互動紀律**（什麼時候不說話、什麼時候不附選單、什麼時候不問）。校準：做「輕量版」時，重點不在簡化結構，在**列出不准做的事**。

**產出**：`.claude/skills/learn/SKILL.md`、`notes/learn-skill-dry-run.md`、README 線二 + 日常節奏各補一行。

**狀態**：進 main。**待使用者裁的一題**：這個 skill 該不該自動觸發？自動＝模糊問題不必記指令（符合「靈活長在意圖層不在工具層」），但檔位 0 每次要付 ~5k token 載入換零收益；只吃 `/learn` ＝零誤觸但要記得叫它。**另一個先不動的**：decision 沉到哪裡（扁平 note 不好被新證據更新，但現在只有 0 個真實決策，等累積 2-3 個再看）。

---

## 2026-09-17 — 從「有什麼公民科技專案」到「自己做一個政治人物任期稽核」

**問**：使用者問台灣政治視覺化有哪些有趣專案，接著問 2026 選舉有什麼可用資料集，再被追問「新竹市長高虹安這任期到底做了什麼」——要求客觀評估、對照選前支票和實際結果、做 before/after 對比、再對照民眾真正在意的議題（交通）跟工程延宕案例。

**做法**：從資源盤點（g0v/READr/窩窩/沃草國會無雙/iVoter/政治獻金系列）逐步收斂成一個實作案例。先查高虹安 2022 政見原文（七大政見主軸+11項動保政見），再查「接手時 vs 現在」的硬數字（公共債務100.17億→0、棒球場、輕軌），中間拆出五層框架（硬資料/政見/時間軸/各方說法/民調）避免混著比。之後被要求對照市民真正在意什麼（一份議員問卷：6成列交通為最優先）、以及工程延宕（查到兩個方向相反的真實案例：中油油庫停滯32年後她上任滿月啟動、拿國家獎；光埔案重劃會承諾3個月變1年半+）。全程用互動 Artifact 呈現、逐項附來源連結，不是純文字報告。

**發現（詳見 `notes/civic-data-taiwan.md`）**
- 政府開放資料平台的「分類」常是生命事件導覽（生育/就學/購屋…）不是主題分類，要找監督類資料得改用組織（機關）分類瀏覽
- 公辦工程 vs 自辦工程課責邏輯不同，但政府公關語言常用同一套總體百分比模糊掉受影響居民的實際感受
- 查無官方逐年可比的城市級數據（新竹市交通壅塞）本身就是監督發現，不是研究失敗——但也要留意「查無」不等於「不存在」

**我踩的坑**：把議員相挺說法「政見落實率破8成」誤植成「債務清償率83%」，直到查到官方零負債公告的具體數字（100.17億→0）才發現兩個說法口徑根本不同。**「篩子第四格：一手 vs 轉述」第四次發作**——這次連「自己第一輪的整理」都要重新對照原始來源才抓到，不只是外部二手來源的問題。跨到一個完全跟 AI 產業無關的領域（台灣地方政治）還是同一個病，說明這條是真的 domain-general 的紀律，不是 AI 產業判讀專屬的技巧。

**產出**：[新竹市長支票簿](https://claude.ai/code/artifact/1e9c9c02-5aa4-48bb-9408-d8dacb1c76ab)（Artifact，五層框架完整實作）、`notes/civic-data-taiwan.md`。

**狀態**：進 main。**還沒搞清楚**：五層方法論是不是新竹剛好資料特別齊全才好用，還是能通用到其他縣市首長——需要再拿一個案例驗證。

---

## 2026-09-18 — 「新竹市長高虹安做得好不好」：一場 /learn，從民調打到財劃法

**問**：使用者叫 `/learn`，題目是「看看新竹市長高虹安做得好不好」。選舉在 11/28，兩個多月後。

**做法**：判檔位 2（四件事），開場一行回述＋列標題＋直接講第一件，全程沒問背景。使用者三次打斷把主線推深：分別看 → 怎麼知道她做了什麼 → **還債又怎麼樣**。最後一次是關鍵。

**發現（完整版在 `notes/evaluating-a-mayor.md`）**
- **停職 17/48 個月＝35%**，直覺是「分開看她的段和代理的段」，但**分開看的結論是分不開**：《天下》連 4 年墊底橫跨兩人。同時推翻「停職才變差」和「換人就會好」。
- **民調三層不能跨層比**：市府自辦 71.6%／匯流 65.3%／天下 52.9 分 22 縣市最後、唯一不及格（遠見 53.4%、倒數第 3）。差別在問法——逐項問 20 項政策永遠比整體評價好看，是問卷設計的必然不是造假。**別漏斜率**：遠見八大面向平均 58.3%、年增 4.1pt，本島增幅次高，水準低但方向正。
- **最硬的一擊：制度變動偽裝成治理績效**。新版財劃法讓竹市財力等級跳兩級（3→1，與台北同級），115 年度統籌分配款 298 億、**年增 214 億**；而代理市長 2025-10 自稱三年減債 78 億 → 100.17 億裡最後約 22 億落在天上掉 214 億那年，只花掉新增款一成，**而那一哩正是招牌**。
- **零負債拆四步**：讀官方自己的限定詞（「受《公共債務法》債限管制的」→ 短期／自償性／潛藏負債不在內）→ 算量級（利息佔歲入約 0.3%，一人一年兩三百）→ 問錢從哪來（同期發消費金 23 億＋福利 12.5 億＝不是撙節）→ 問是投入還是產出（舉債額度是選擇權；歲入 467.27 億、歲出 437.16 億、**賸餘 30.11 億**＝錢沒花出去）。
- **歸因分級（本場主要產物，與 09-17 五層框架正交）**：A 市長說了算（資本支出佔比／公托名額／決標）／B 能影響（事故率／PM2.5／通勤）／C 幾乎管不到（房價／醫療量能／人口／所得）。**看 A 級判官，看 C 級判城市**——政治攻防永遠在 C 級。例證：2024 台大新竹分院升格為竹竹苗首家醫學中心，是這十年最大醫療變化，但那是中央與台大的事。

**我踩的坑（篩子第四格第五次發作，這次很丟臉）**：在「她復職後有可查的交付」那步，我把「清償百億公債」直接當硬資料列出來，**沒先拆它**——完全就是我同一場前一則訊息才剛教過的病（把官方宣稱當硬資料）。是使用者問「零負債對市民收益到底是什麼」才逼出財劃法那層。**診斷：我對「數字」和「有查證管道的數字」設了同一個信任等級，只要它來自官方新聞稿且有小數點就放行。**

**預測帳**：查之前先寫了猜測——「二審會大幅改判、她會復職」✅對；**沒猜到的是民調會分裂成兩個世界**（自辦 71.6% vs 天下墊底），那才是這題真正的難點。校準：評估政治人物時，我的預設注意力放在司法/事件，但真正的資訊在**指標口徑**上。

**環境限制**：`data.gov.tw`、新竹市社會經濟統計平台、`cw.com.tw`、`news.ltn.com.tw` 全被 egress 擋（env-403 第 4 次），**A 級指標一條歷年序列都沒撈到**，全篇降級為搜尋摘要層，已在 note 內標註。主動棄用一組單位邏輯對不上的房價數字。
