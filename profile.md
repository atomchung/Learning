...
- **AI 產業判斷 / 投資訊號** — 讀信號不讀數字、開源商品化計時器、價值流向、中國開源、模型進步軌跡;最新:Anthropic 5–6 月 blog(S-1 $47B run-rate 主力 Claude Code、Glasswing/Mythos「強到不敢公開」、6/4 呼籲全行業放慢開發,`notes/anthropic-blog-2026-06.md`);前次:eval 生態位(`notes/eval-ecosystem-niche.md`)
- **MSFT × OpenAI super app** — 平台×模型乘積、Graph/Entra 護城河、Copilot 滲透真假
- **學習 / 教育加速** — Alpha School、learning roadmap、skills/eval 設計
- **知識系統怎麼設計才真的複利** — 元主題，就是這個 repo 本身（你正在用它）。最新:啟動「遞迴改進 harness」(Issue #6)——從 L1 記憶迴圈進化到 L2 自我改進規則,已接 `meta/defects.md` + `/meta-review` skill
...
- 遞迴改進 harness（Issue #6）：基礎設施已接（defects 日誌 + meta-review）。下一步是**累積缺陷**——日常 session 若 boot/retrieval/rot/merge 出包就記一筆，攢一批後跑第一次 `/meta-review`。能刪規則的 RSI 才是對的 RSI。
