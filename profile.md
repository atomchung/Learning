...
updated: 2026-07-05
last-session: 2026-07-05 Fable 視窗執行日：26 專案掃描分層（fable-limited-window-strategy.md）→ 前三任務執行（Issue #6 梳理/預測帳壓測/cwc-workshops 對讀）＋ Fable vs Opus 4.8 同卡 A/B。結論：單卡綜合 Opus 夠好（重疊 70-80%、零幻覺）,Fable 價值在跨脈絡連線與帳本意識；升卡候選「檢查只設單側,系統就往單側漂」（三脈絡交會）；預測帳 10 條修改待裁。四 note 進 main,詳 inbox 2026-07-05（兩條）。同日稍早：Thariq「找未知」筆記
...
- **AI 產業判斷 / 投資訊號** — 讀信號不讀數字、開源商品化計時器、價值流向、中國開源、模型軌跡。核心：記憶體＝三個市場非一個（commodity DRAM/HBM/NAND 定價邏輯不同，混講必誤判）；元判斷「商品週期盯 capex 紀律不盯需求故事」。最新(07-05 壓測修正)：06-28 曾記「84.9% 毛利率＝框架強印證」——**撤回，改判「與框架相容」**（毛利率是框架的預測項不是檢驗項，當印證＝循環論證；investment_note 同期 stress test 判多頭證據偏軟）。改盯證偽觸發：SCA 合約弱化／capex 從升級轉擴產／庫存週數>15／CXMT 滲透。**thesis 對 ≠ sizing 對，組合風險另判**（`notes/memory-industry-map.md`、investment_note `research/20260627_memory_decyclicalization_stress_test.md`、inbox 2026-07-05）。
...
- **Fable 視窗待辦（07-05 更新）**：掃描與前三任務已完成（見 `notes/fable-limited-window-strategy.md` 進度節）。剩兩條都動別的 repo、開新 session 做：fomo-kernel 鏡片對抗測試（工具已備：cwc audit.md 三層稽核＋per-task auditor prompt）、personal_os 架構 audit。另待裁：預測帳 10 條修改＋both-directions 升卡（`notes/prediction-ledger-stress-test.md`）。
