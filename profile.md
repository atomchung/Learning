...
- **loop eng 會被 harness 吃掉還是長出外圍治理層？**（2026-06-27，`check:2026-12`）追蹤中＝harness>model 的對沖線。六積木現都在 harness 廠手裡；若連 loop 都被原生吃掉＝護城河更深，若冒出獨立 loop 成本/安全治理層或 verifier-as-a-service＝槓桿再外移一層（同 `llm-call-niches` 結構，外圍才是可投資表達式）（`notes/loop-engineering.md`）。
