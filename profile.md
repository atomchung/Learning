...
- **AI agents / coding agents** — 最深（~15 卡）。核心：harness>model、沙箱、記憶架構、人機協作、多 agent 編排。最新(2026-06-20)：openclaw vs Hermes 個人 agent 框架雙雄——差異化全在 harness 不在 model、記憶是勝負手（`notes/openclaw-hermes-personal-agent-frameworks.md`）。
...
- **遞迴改進 harness（Issue #6）** 追蹤中＝最活躍的元線。基礎設施已接（`meta/defects.md` + `/meta-review` + R1 防自評）。借鑒 ledger `meta/borrowable-patterns.md`：**B1–B3 已落地 CLAUDE.md**（蒸餾時機前移／連坑一起記／≥3 次出現升級）；**B8 已落地**（profile 三層瘦身，即本檔）；**B9（reflection 先問問題+importance）、B10（外部內容衛生）提案中**。待辦：累積缺陷後跑首次 `/meta-review` 驗證。R2/R3：CLAUDE.md 行數要壓回 <200。能刪規則的 RSI 才是對的 RSI。**下次明確起點（2026-06-20 定）**：先讓系統跑一陣，使用時把「漏接/給錯」隨手標進 `meta/defects.md`（行尾 `@user`）；攢幾筆後跑首次 `/meta-review`，一次反向砍規則 + 驗證 B1–B8 有沒有真用。別在跑 review 前繼續加借鑒點（這輪已加 4 條進 CLAUDE.md，285 行）。
