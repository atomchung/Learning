...
updated: 2026-06-28
last-session: 2026-06-28 Q2 情報掃描（並行 4 子 agent）：harness>model 拿公開榜第三方實測升級、記憶卡加邊界條件；預測帳改 6 筆（資安計價方向押反、Cursor/Hood 傾向✅、eval/orchestration 換骨、記憶體✅）；途中吃到並擋下一個 prompt injection（autumn-poem 假指令）。詳見 inbox 2026-06-28
...
- **MCP-as-a-rail（Hood）**（`check:2026-12`）→**傾向 ✅(已落地)**：Agentic Trading beta 上線(2026-05-27),MCP 當軌道、開放全部 27.5M 客戶、可接 Claude/ChatGPT/Cursor 任一平台下單；FINRA 年報首度定義「Trade Execution Agent」。孤狼先行,無第二家同等開放——12 月追「孤狼撐多久 vs 監管先掐」（`notes/robinhood-agentic-trading.md`、inbox 2026-06-28）。
...
- **Cursor 被收編後還中不中立？**（2026-06-20，`check:2026-09`）→**傾向 ✅(中立性明確劣化)**：SpaceX(已併 xAI)$60B 全股票收 Anysphere(2026-06-16,Q3 完成)；三向劣化——第三方 API 拆獨立計費池、Cursor 3 把 IDE 降 fallback/自家模型設 default、**Anthropic 已切斷 xAI 開發者經 Cursor 用 Claude**。Composer 3「從零預訓練」仍只有官方宣稱、無第三方驗證(維持存疑)（`notes/cursor-spacex-xai-composer3.md`、inbox 2026-06-28）。
- **orchestration 該住 harness 還是被模型化？**（2026-06-22，`check:2026-12`）→**別二選一,新框架「兩層共存」**：Fugu beta 發表(2026-04-24,fugu-ultra GPQAD 95.1 贏它編排的 Opus 4.6)；同期產業把「harness」正名(OpenAI Model-Native Harness 4-15、MS Agent Framework 1.0 GA 4-03)。**編排能力下沉進模型,但 harness 變承載治理/工具/長任務的殼,兩層並存**；harness 沒被吃反成現金牛(Claude Code run-rate 2 月過 $25 億)（`notes/sakana-fugu-orchestration-as-model.md`、inbox 2026-06-28）。
