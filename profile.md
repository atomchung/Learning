...
updated: 2026-07-13
last-session: 2026-07-13 研究 Anthropic Claude 認證計畫(Associate/Developer/Architect,Partner Network 底下)——免費路徑釐清(Academy 課程證書 vs 監考認證是兩件事,個人身分大機率沒有免費監考路)、ROI 論述源頭多是賣備考課程站(套「讀信號不讀表面數字」濾掉)、模擬考生態異常(新證照上線幾個月冒一堆同名 SEO 站,建議別留個資)、拿 Architect Foundations 5 domain 權重對照既有卡片估算重疊 ~70%>Developer 軌,選定該軌;撈 Anthropic engineering blog 5 篇官方 best practice,兩處直接命中既有判斷(Domain 5 JIT retrieval = 卡 `precompile-to-local-index` 官方原始出處背書;Domain 3 SKILL.md 拆分 = fomo-kernel 07-08 瘦身構想的官方驗證);誠實缺口分析(強項 62% Domain1+3+5 架構判斷層/真缺口 38% Domain2+4 MCP tool design 實作與 prompt engineering API 層技巧筆記空白),設計三階段讀書計畫。WebFetch 對外部站全 403(環境 egress 白名單),降級搜尋摘要。全沉澱進 `notes/claude-certification.md`。(07-08 fomo-kernel 架構深挖仍是開放候選池項目,見下方)
