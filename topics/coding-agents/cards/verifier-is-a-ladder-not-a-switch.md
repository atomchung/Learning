...
- ↔ 姊妹判斷 [notes/workflow-into-training.md](../../../notes/workflow-into-training.md)（同日另一場並行 session 獨立產出）：那篇講「**可驗證性 × 頻率 × 穩定性**決定一個能力沉進模型、留在 harness、還是當中間膠水」＝**縱向**的層歸屬；本卡講「verifier 站哪一階 × 任務吃不吃得到紅利」＝**橫向**的 loop 能跑多遠。兩者共用同一個前提——**沒有可靠的驗證訊號，就沒有下沉，也跑不遠**。那篇的兩張候選卡（A 移動邊界／B no-train 節流）尚未建。
