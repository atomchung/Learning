...
2026-08-05→08-09 [boot-miss] 08-05 那場 session 寫了 `notes/product-hunt-agent-services-2026-08.md`（341 行，判斷 Prelint/Pushary/AnySearch 值得試、Unabyss 因雙真相來源否決）並 commit 進 main，**但沒更新 profile.md、也沒寫 inbox.md**——睡前三拍只做了「寫 note」沒做「回寫記憶層」。後果：08-09 這場開機讀 profile 時那次 session 等於不存在，`last-session` 還停在 07-26，差點在掃描時漏掉「Product Hunt 那批工具是否需要更新」這個已知脈絡（本次是靠 `ls -lt notes/` 逐檔看修改時間才發現）。**與既有 boot-miss 的差異**：以往 boot-miss 是「開場漏讀」，這次是**寫入端漏做導致下游必然 boot-miss**——記憶層的斷點在生產者不在消費者。根因候選：① 該場 commit author 是 TING 本人帳號（seal161285），可能不是走 Claude session 的完整三拍流程，規則對「人自己寫 note」這條路徑沒有覆蓋 ② CLAUDE.md 把「睡前必做」寫在記憶層段落，但沒有機制層兜底（無 hook、無 gate）。**候選解**：(a) 加 SessionStart 或 Stop hook 檢查「本 session 若動過 notes/ 但 profile.md 未變更則提醒」(b) 接受人工路徑不回寫，改成每次 boot 時比對 `git log --since=<profile.updated>` 找出 profile 未涵蓋的 commit（成本低、不依賴自律，本次即是手動做了這件事）。交 /meta-review 評估。 @user

2026-08-09 [credibility-miss] 預測帳「安全/紅隊 eval」把「EU AI Act 8/2 大限催出獨立紅隊顧問市場」寫進 profile 當 ✅ 傾向依據，但 8/2 真正生效的只有 Article 50 透明度義務；**高風險義務（紅隊需求所繫）已被 Digital Omnibus（Regulation (EU) 2026/1744，7/24 刊登、7/27 生效）延到 2027-12-02**。錯因＝**採信了合規顧問/資安 vendor 的行銷內容（$125-200/hr、$130-200k/年那組數字），沒有回查官方公報確認法條現況**——那些內容到今天仍在大流通，講的是被延後掉的那條線。**這是既有三層篩子攔不住的一類**：信號 vs 表面數字（過不了，它就是信號敘事）、估計值 vs 實測值（過不了，市場費率是真實報價）、作者自報 vs 獨立複現（過不了，多家獨立顧問公司都在講同一件事）——**多個獨立來源同時錯，是因為它們共用同一個過期前提**。已即時固化：profile 立**篩子第四格「敘事 vs 法條現況」**（凡涉法規催生市場的判斷一律回查官方公報/一手法條）。留給 /meta-review 的問題：`credibility-miss` 是否該正式立類（07-04 已列為候選、當時「再犯即立類」，本次即為再犯）。 @claude
...
2026-08-10 [credibility-miss ← 立類，第 2 次觸發] 使用者傳 FB 圖卡（NVIDIA CMX 微秒層）要求理解。查「市場反應」時撈到 invezz／tradingkey 的市場評論文章，其導言寫「following the company's mixed guidance... concerns about a more prolonged glut as supply outpaces demand in smartphone and PC segments」，**我把它讀成「SanDisk 說 NAND 會供過於求更久」並寫進回覆**。使用者追問「不是缺貨嗎？AI 需求這麼旺怎麼會過剩」→ 回頭讀法說會，管理層實際定調是**結構性稀缺**（FY27 供給 >50% 已鎖長約、毛利率指引 83–85%、NBM 長約 floor pricing 最低營收 $93.9B、加碼 $14B 庫藏股），**與我轉述的方向相反**。已在回覆中更正並寫進 `notes/nvidia-cmx-kv-cache-tier.md` §七。

**根因＝歸屬錯誤，不是可信度誤判。** 市場評論文章的句式（「在公司財報後，市場擔心 X」）在語法上長得像公司陳述，我把「市場擔心 X」讀成「公司說 X」。**現有三把篩子（信號 vs 表面數字／估計值 vs 實測值／作者自報 vs 獨立複現）＋第四格（敘事 vs 法條現況）管的都是「誰的話可信」，全都預設了「這句話是誰說的」已經確定。這次漏的是更前面一階：attribution 本身。**

**候選解（交 /meta-review）**：立**第五格篩子「一手 vs 轉述」**——凡要寫「某公司/某人說了什麼」，來源必須是逐字稿／新聞稿／財報／官方公報；市場評論文章只能用來引用「市場如何反應」，且兩者在文中要分開標註。這與第四格（敘事 vs 法條現況，2026-08-09 立）同源——**兩次都是拿二手敘事當一手事實**，第四格是法規版、這次是財報版，可考慮合併成一格「一手來源強制」而非再開一格（能刪規則的 RSI 才是對的 RSI，見 Issue #6 R2/R3）。

**正向觀察**：這個坑是**使用者的追問逼出來的**——他問「AI 需求這麼旺怎麼會過剩」時，我若照原答案圓下去就會編出一套錯的因果。**「使用者覺得哪裡怪」是比自查更強的 verifier**，符合 `verifier-is-a-ladder-not-a-switch` 第三階「驗證訊號要在被驗證者控制範圍之外」。 @user
