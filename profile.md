...
updated: 2026-07-03
last-session: 2026-07-03（第二場）橋水×TML 專家判斷微調：三信號接既有線（稀缺資產＝專家標註資料、Qwen 成華爾街生產選型、TML＝管道生態位）；方法論真亮點「專家只標邊界案例」→ B14 已落地兩個 skill（首個 validation-gated 修改）。詳見 `notes/tml-bridgewater-expert-judgment-finetune.md`、inbox 2026-07-03。同日前場 best-practice 掃描（B11–B13）見 inbox 2026-07-03 上一條
...
- **遞迴改進 harness（Issue #6）**（`check:2026-07`）追蹤中＝最活躍的元線。基礎設施已接（`meta/defects.md` + `/meta-review` + R1 防自評）。借鑒 ledger `meta/borrowable-patterns.md`：**B1–B3 已落地 CLAUDE.md**（蒸餾時機前移／連坑一起記／≥3 次出現升級）；**B8 已落地**（profile 三層瘦身，即本檔）；**B9（reflection 先問問題+importance）、B10（外部內容衛生）提案中**。待辦：累積缺陷後跑首次 `/meta-review` 驗證。R2/R3：CLAUDE.md 行數要壓回 <200。能刪規則的 RSI 才是對的 RSI。**下次明確起點（2026-07-03 更新）**：直接跑首次 `/meta-review`——設計輸入已齊（R1 防自評閘 + B11 validation-gate/否決 buffer + B13 Issue #7 裁決材料 + defects 5 筆含 2 筆 @user），一次反向砍規則（CLAUDE.md 已 290 行，>200 行整塊被忽略）+ 驗證 B1–B10 有沒有真用。B11–B13 只進 ledger 沒動 CLAUDE.md（守住「review 前不加規則」）；B14（用戶只審邊界案例，2026-07-03 第二場）經用戶點頭破例落地兩 skill＝首個 validation-gated 試點，訊號見 ledger。**→ 結算訊號**：跑完首次 `/meta-review` 且能反向砍掉 ≥1 條規則（CLAUDE.md 行數實際下降）。
